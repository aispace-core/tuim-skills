# Normal PPT Agent Teams + Public Preview Notes

Use these notes when evolving Normal PPT from an outline Skill into a public Agent Teams workflow.

## Product shape

Normal PPT should be positioned as the **main orchestrator Agent** for PPT production, not as a renderer or a bundle of slide templates.

```text
Normal PPT Agent / Main Orchestrator
...
The main Agent loads `normal-ppt`; each specialist Agent loads exactly the Skill it needs. The main Agent owns AST, routing, commands, manifests, and final QA.
...
- Do not let downstream PPT Skills consume raw source directly. Normal PPT should first emit the AST production contract.
- Do not frame Normal PPT as a fixed set of 4 HTML PPT Skills. Frame it as an orchestrator that can route to many Skills.
